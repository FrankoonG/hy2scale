...
func (a *App) updateChapSecrets() {
	...
	for _, u := range cfg.Users {
		// Skip users disabled globally OR explicitly revoked from l2tp.
		// Without this skip, pppd would still accept the credentials and
		// the per-proxy disable wouldn't be hot-effective for L2TP.
		if u.Enabled && u.IsProxyEnabled("l2tp") {
...
func (a *App) handleTransparent(conn net.Conn) {
	...
	user, err := a.LookupUser(username, "", "l2tp")
	if err != nil {
